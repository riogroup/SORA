...
def _fit_results(result, parameters, residual, bootstrap=None, lib='scipy', sigma=1, method='Unavailabe'):
    ...
    output.initial_values = np.array(parameters.get_values())
    ...
    output.best_fit = np.array(result.x)
    ...
    stderr = np.ones(len(result.x))*inf
    ...
    if hasattr(result, 'jac') and (method == 'least squares'):
        ...
        try:
            ...
            stderr = np.sqrt(diag(output.covar))*sigma #sigma is normalized in this case
        except:
            output.covar = None
